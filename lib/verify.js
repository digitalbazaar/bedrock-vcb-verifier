...
import {parsePdf417} from './pdf417.js';
...
export async function verifyVcb({data, format, getVerifyOptions} = {}) {
  try {
    ...
    } else if(format === BARCODE_FORMATS.PDF417) {
      const {jsonldDocument, hashed} = await parsePdf417({
        pdf417String: data,
        documentLoader,
        typeTableLoader,
      });
      credential = jsonldDocument;
      extraInformation = hashed;
    } else {
      throw new Error(`Barcode format, ${format}, not yet supported.`);
    }
    return verifyCredential({credential, extraInformation});
  } catch(cause) {
    throw new BedrockError(
